...
// File: contracts\PBRMaker.sol

pragma solidity 0.6.12;


// This contract handles "serving up" rewards for xPBR holders by trading tokens collected from fees for PBR.
contract PBRMaker  is Ownable {
    using SafeMath for uint;
    using SafeERC20 for IERC20;
    IUniswapV2Router02 public routerv2;
    IUniswapV2Factory public factory;
    address public pbrSafe;
    address public pbr;
    address public weth;
    ...
    function removeLiqidity(address token0, address token1, uint amount) public {
        require(operators[msg.sender], 'PBRMaker: no permistion');
    ...
    function marketBuyPBRWithETH(address[] calldata path, uint amount, uint deadline) public {
      require(operators[msg.sender], 'PBRMaker: no permistion');
      require(path[path.length - 1] == pbr, 'PBRMaker: wrong path');
      for (uint i = 1; i < path.length - 1; i++) {
        require(acceptTokens[path[i]], 'PBRMaker: wrong path');
      }
    ...
    function marketBuyPBRWithToken(address[] calldata path, uint amount, uint deadline) public {
      require(operators[msg.sender], 'PBRMaker: no permistion');
      address token = path[0];
      if (token == pbr) {
          IERC20(token).safeTransfer(pbrSafe, amount);
      } else  {
        require(path[path.length - 1] == pbr, 'PBRMaker: wrong path');
        for (uint i = 1; i < path.length - 1; i++) {
          require(acceptTokens[path[i]], 'PBRMaker: wrong path');
    ...
    function convert(address token0, address token1) public {
        ...
        if (IERC20(weth).balanceOf(address(this)) > 0) {
          ...
          _swap(weth, pbr, IERC20(weth).balanceOf(address(this)));
        }

        if (IERC20(pbr).balanceOf(address(this)) > 0) {
          IERC20(pbr).safeTransfer(pbrSafe, IERC20(pbr).balanceOf(address(this)));
        }
    }
    ...
    function _toBaseToken(address token, uint amountIn) internal {
      ...
      if (_canSwap(token, pbr)) {
        _swap(token, pbr, amountIn);
      }
