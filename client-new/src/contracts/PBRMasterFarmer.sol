...
// PBRToken with Governance.
contract PBRToken is ERC20("PBR", "PBR"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(now <= expiry, "PBR::delegateBySig: signature expired");
    ...
    constructor() public {
        ...
        _mint(msg.sender, _initial_supply); // Mint 5000 PBR for bounty program
        _moveDelegates(address(0), msg.sender, _initial_supply);
    }
}

interface IMigratorToPBRSwap {
    // Perform LP token migration from legacy UniswapV2 to PBRSwap.
...
// PBRMasterFarmer is the master of PBR. He can make PBR and he is a fair guy.
//
...
contract PBRMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accPBRPerShare; // Accumulated PBRs per share, times 1e12. See below.
    }
    ...
    // Bonus muliplier for early PBR makers.
    ...
    uint256 public burnPercent = 0; // init 0% burn pbr

    uint256 public lockFromBlock;
    uint256 public lockToBlock;

    // The migrator contract. It has a lot of power. Can only be set through governance (owner).
    IMigratorToPBRSwap public migrator;
    ...
    event SendPBRReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    event Lock(address indexed to, uint256 value);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "PBRMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function setMigrator(IMigratorToPBRSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 pbrForBurn;
        uint256 pbrForDev;
        uint256 pbrForFarmer;
        (pbrForBurn, pbrForDev, pbrForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);

        if (pbrForBurn > 0) {
            pbr.mint(address(this), pbrForBurn);
            pbr.burn(pbrForBurn);
        }

        if (pbrForDev > 0) {
            // Mint unlocked amount for Dev
            pbr.mint(devaddr, pbrForDev.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
            // Mint locked amount for Dev
            //For more simple, I lock reward for dev if mint reward in bonus time
            pbr.mint(address(this), pbrForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            farmLock(devaddr, pbrForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
        }
        pbr.mint(address(this), pbrForFarmer);
        pool.accPBRPerShare = pool.accPBRPerShare.add(pbrForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forBurn, uint256 forDev, uint256 forFarmer) {
        ...
        uint256 pbrCanMint = pbr.cap().sub(pbr.totalSupply());

        if (pbrCanMint < amount) {
            forBurn = 0;
            forDev = 0;
            forFarmer = pbrCanMint;
        }
        else {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 pbrForFarmer;
            (, , pbrForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accPBRPerShare = accPBRPerShare.add(pbrForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accPBRPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accPBRPerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = pbr.balanceOf(address(this));
            ...
            if(pending > 0) {
                pbr.transfer(msg.sender, pending.mul(100 - PERCENT_LOCK_BONUS_REWARD).div(100));
                ...
                emit SendPBRReward(msg.sender, _pid, pending, lockAmount);
            }

            user.rewardDebt = user.amount.mul(pool.accPBRPerShare).div(1e12);
        }
    }

    // Deposit LP tokens to PBRMasterFarmer for PBR allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "PBRMasterFarmer::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from PBRMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "PBRMasterFarmer::withdraw: not good");
    ...
    function farmLock(address _holder, uint256 _amount) internal {
        require(_holder != address(0), "ERC20: lock to the zero address");
        require(_amount <= pbr.balanceOf(address(this)), "ERC20: lock amount over blance");
        _locks[_holder] = _locks[_holder].add(_amount);
        _totalLock = _totalLock.add(_amount);
        if (_lastUnlockBlock[_holder] < lockFromBlock) {
            _lastUnlockBlock[_holder] = lockFromBlock;
    ...
    function unlock() public {
        ...
        uint256 amount = canUnlockAmount(msg.sender);
        // just for sure
        if (amount > pbr.balanceOf(address(this))) {
            amount = pbr.balanceOf(address(this));
        }
        pbr.transfer(msg.sender, amount);
        _locks[msg.sender] = _locks[msg.sender].sub(amount);
        _lastUnlockBlock[msg.sender] = block.number;
        _totalLock = _totalLock.sub(amount);
    }
