  ...
  return (
    <StyledApy>
      ...
      <StyledBox className="col-7">
        ...
        <StyledContent>
          {stakedValue && stakedValue.usdValue
            ? parseFloat(
                Web3.utils.fromWei(
                  stakedValue.usdValue.toFixed(0).toString(),
                  'ether',
                ),
              )
                .toFixed(0)
        ...
        <StyledEquility>
          {totalStake ? getBalanceNumber(totalStake).toFixed(2) : '~'}{' '}
          <span style={{ fontSize: 12 }}>{symbolShort} LP</span>
        </StyledEquility>
      </StyledBox>
      <StyledBox className="col-3">
        <StyledLabel>Avg. reward per block</StyledLabel>
        <StyledContent>
          {newReward ? getBalanceNumber(newReward).toFixed(3) : '~'} PBR
        </StyledContent>
        <StyledEquility>
          ≈{' '}
          {stakedValue &&
