...
contract PolkaBridgeMasterFarm is Ownable, ReentrancyGuard {
    ...
    event EmergencyWithdraw(address indexed user, uint256 pid);
    event StopPool(uint256 pid);
    event ActivePool(uint256 pid);
    event ChangeMultiplier(uint256 pid, uint256 multiplier);

    //
    //pool Info
    PoolInfo[] public poolInfo;
    mapping(address => bool) public mappingPool;
    ...
    function add(
        ...
        mappingPool[address(_lpToken)] = true;
        poolInfo.push(
            PoolInfo({
                lpToken: _lpToken,
                tokenA: _tokenA,
    ...
    function getChangePoolReward(uint256 _pid, uint256 _totalMultiplier)
        public
        view
        returns (uint256)
    {
        ...
        changePoolReward = (
            (currentPoolBalance.sub(totalLastPoolReward))
                .mul(poolInfo[_pid].multiplier)
                .mul(1e18)
        )
            .div(_totalMultiplier);
    ...
    function massUpdatePools() public {
        ...
        for (uint256 pid = 0; pid < length; pid++) {
            if (poolInfo[pid].isActived) {
                uint256 changePoolReward =
                    getChangePoolReward(pid, totalMultiplier);
    ...
    function getTotalLastPoolReward() public view returns (uint256) {
        ...
        for (uint256 pid = 0; pid < length; pid++) {
            if (poolInfo[pid].isActived) {
                total = total.add(poolInfo[pid].lastPoolReward);
            }
        }
        return total;
    }
    ...
    function updatePool(uint256 _pid, uint256 _changePoolReward) internal {
        ...
        pool.lastPoolReward = pool.lastPoolReward.add(_changePoolReward);
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            temptAccPBRPerShare = pool.accPBRPerShare.add(
                (
                    getChangePoolReward(_pid, totalMultiplier).mul(1e18).div(
                        lpSupply
                    )
                )
            );
    ...
    function changeMultiplier(uint256 pid, uint256 _multiplier)
        public
        onlyOwner
    {
        PoolInfo storage pool = poolInfo[pid];
        ...
        emit ChangeMultiplier(pid, _multiplier);
    }
    ...
    function countTotalMultiplier() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            if (poolInfo[i].isActived)
                totalMultiplier = totalMultiplier.add(poolInfo[i].multiplier);
        }
    ...
    function avgRewardPerBlock(uint256 _pid) public view returns (uint256) {
        ...
        uint256 totalReward =
            poolInfo[_pid]
                .totalRewardClaimed
                .add(poolInfo[_pid].lastPoolReward)
                .add(changePoolReward);
        uint256 changeBlock;
        if (block.number <= poolInfo[_pid].lastRewardBlock) {
            changeBlock = poolInfo[_pid].lastRewardBlock.sub(
                poolInfo[_pid].startBlock
            );
        } else {
